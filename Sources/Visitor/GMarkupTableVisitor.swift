...
public struct GMarkupTableVisitor: MarkupVisitor {
    ...
    public mutating func visitTableBody(_ tableBody: Table.Body) -> GMarkTable {
        ...
        // 表格结束后的空行/分隔符不应该成为表格数据。若解析结果尾部出现全空行，
        // 这里只裁掉尾部全空行，不影响表格中间用户显式输入的空行。
        while let lastRow = markTable.bodys?.last, isEmptyTableRow(lastRow) {
            markTable.bodys?.removeLast()
        }
        return markTable
    }
    ...
    private func isEmptyTableRow(_ row: [NSAttributedString]) -> Bool {
        return row.allSatisfy { attribute in
            attribute.string.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty
        }
    }
}
