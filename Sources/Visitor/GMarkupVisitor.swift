...
public struct GMarkupVisitor: MarkupVisitor {
    ...
    public mutating func visitParagraph(_ paragraph: Paragraph) -> NSAttributedString {
        if let htmlFragment = paragraph.renderableHTMLFragment {
            let attributedString = CommonHTMLAttribute.parseHTMLWithSwiftSoup(from: htmlFragment, style: style)
...
//        return parseHTMLWithSwiftSoup(html.rawHTML)
        return CommonHTMLAttribute.parseHTMLWithSwiftSoup(from: html.rawHTML, style: style)
    }
}
