...
open class MarkdownAttachingBehavior: NSObject {
    ...
    private var needsUpdateAttachedSubviews = false
    private var needsLayoutAttachedSubviews = false
     ...
     */
    @objc
    open func setNeedsUpdateAttachedSubviews() {
        guard !needsUpdateAttachedSubviews else { return }
        needsUpdateAttachedSubviews = true
        DispatchQueue.main.async { [weak self] in
            guard let self = self else { return }
            self.needsUpdateAttachedSubviews = false
    ...
    @objc
    open func setNeedsLayoutAttachedSubviews() {
        guard !needsLayoutAttachedSubviews else { return }
        needsLayoutAttachedSubviews = true
        DispatchQueue.main.async { [weak self] in
            guard let self = self else { return }
            self.needsLayoutAttachedSubviews = false
            self.layoutAttachedSubviews()
        }
    }
    ...
    open func layoutAttachedSubviews() {
        ...
        for (attachment, range) in attachmentRanges {
            ...
            UIView.performWithoutAnimation {
                if !view.frame.isAlmostEqual(to: integralRect, scaleFactor: scaleFactor) {
...
extension MarkdownAttachingBehavior: NSLayoutManagerDelegate {
    ...
    public func layoutManager(_ layoutManager: NSLayoutManager, didCompleteLayoutFor textContainer: NSTextContainer?, atEnd layoutFinishedFlag: Bool) {
        if layoutFinishedFlag {
            self.setNeedsLayoutAttachedSubviews()
        }
    }
}
...
extension MarkdownAttachingBehavior: NSTextStorageDelegate {
    ...
    public func textStorage(_ textStorage: NSTextStorage, didProcessEditing editedMask: NSTextStorage.EditActions, range editedRange: NSRange, changeInLength delta: Int) {
        if editedMask.contains(.editedAttributes) {
            self.setNeedsUpdateAttachedSubviews()
        } else if editedMask.contains(.editedCharacters),
                  !attachedProviders.isEmpty || textStorage.containsSubviewAttachment(in: editedRange) {
            self.setNeedsUpdateAttachedSubviews()
        }
    }
}
...
private extension NSAttributedString {
    ...
    func containsSubviewAttachment(in range: NSRange) -> Bool {
        guard length > 0, range.location != NSNotFound else { return false }
        let location = min(max(range.location, 0), length - 1)
        let requestedLength = max(range.length, 1)
        let safeEnd = range.location > Int.max - requestedLength ? length : range.location + requestedLength
        let upperBound = min(max(safeEnd, location + 1), length)
        let effectiveRange = NSRange(location: location, length: upperBound - location)
        var containsAttachment = false
        enumerateAttribute(NSAttributedString.Key.attachment, in: effectiveRange, options: []) { value, _, stop in
...
private extension CGRect {
    ...
    func isAlmostEqual(to rect: CGRect, scaleFactor: CGFloat) -> Bool {
        let tolerance = max(1.0 / max(scaleFactor, 1), 0.5)
