...
class GMarkMermaidBrowser: UIViewController {
    ...
    private func setupUI() {
        ...
        let doneButton = UIBarButtonItem(barButtonSystemItem: .done, target: self, action: #selector(dismissBrowser))
        let spacing = UIBarButtonItem(barButtonSystemItem: .fixedSpace, target: nil, action: nil)
        spacing.width = 16  // 控制间距大小
        navigationItem.rightBarButtonItems = [spacing, doneButton]
...
extension GMarkMermaidBrowser: WKNavigationDelegate, WKScriptMessageHandler {
    ...
    private func saveImageToPhotoLibrary(_ pngDataString: String) {
        let components = pngDataString.split(separator: ",", maxSplits: 1, omittingEmptySubsequences: true)

        guard components.count > 1,
              let imageData = Data(base64Encoded: String(components[1]), options: .ignoreUnknownCharacters) else {
