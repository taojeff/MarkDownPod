...
public class GMarkTableView: UIView, UIScrollViewDelegate {
    ...
    override public func layoutSubviews() {
        ...
        rect.size.width = max(bounds.width - rect.minX - style.borderWidth, 0)
        trScrollView.frame = rect

        rect = tlScrollView.frame
        rect.origin.y = numberOfLockingRows > 0 ? rect.maxY + style.rowGap : rect.maxY
        rect.size.height = max(bounds.height - rect.minY - style.borderWidth, 0)
        ...
        size.width = max(size.width - (style.borderWidth * 2 + style.colGap + blScrollView.frame.width), 0)
        size.height = max(size.height - (style.borderWidth * 2 + style.rowGap + trScrollView.frame.height), 0)
        brScrollView.contentSize = size

        // 不要在 layoutSubviews 内修改自身 frame。MarkdownTextView / UICollectionViewCell
        // 已经通过 attachment bounds 或 cell layout 给了表格容器宽度；这里再改 self.frame
        // 会触发父视图重新布局，流式表格内容下容易形成反复布局/抖动。
